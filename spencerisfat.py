...
def enumerateAllPossibilities(adjMatrix, colorArray, threshold):
    possibilities = [] # list of [pathLength, [path]]
    cityNum = len(adjMatrix)
    ucityList = [i for i in range(0, len(colorArray))]
    for s in range(0, cityNum):
        sColor = colorArray[s]
        if sColor == 'R':
            possibilities.append(visit(adjMatrix, s, sColor, 1, 0, 1, threshold, ucityList, [s], 0)) # within DFS, if sum of path length exceeds threshold, return None
        elif sColor == 'B':
            possibilities.append(visit(adjMatrix, s, sColor, 0, 1, 1, threshold, ucityList, [s], 0)) # within DFS, if sum of path length exceeds threshold, return None
        else:
            raise Exception("invalid color for starting node")
        # updating the threshold to be tighter every iteration
        threshold = min(threshold, possibilities[s][0])
...
def legalNeighbors(currCity, currColor, currColorCnt, rCount, bCount, ucityList, adjMatrix, colorArray, routeTrack):
    ...
    for i in range(0, len(ucityList)):
        ...
        or (colorArray[ucityList[i]] == currColor and currColorCnt >= 3) \
